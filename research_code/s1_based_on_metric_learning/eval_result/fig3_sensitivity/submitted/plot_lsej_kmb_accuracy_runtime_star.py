...
读取同目录 sensitivity.csv，另存带 _star 后缀的 PDF/PNG，不覆盖原版本。
星标表示论文采用的默认参数 K=3、M=4、B=4，不表示该曲线的最高精度点。
...
OUTPUT_STEM = "lsej_kmb_accuracy_runtime_star"  # 改数据集时可同步改输出名。
...
FONT_SIZE = 9
PNG_DPI = 450
PA_COLOR = "#005A8C"       # 加深的蓝色，提高缩放及打印后的辨识度。
TIME_COLOR = "#B84700"     # 加深的橙色，提高缩放及打印后的辨识度。
PA_TICK_STEP = 4
TIME_TICK_STEP = 2
DEFAULT_PARAMETERS = {"K": 3, "M": 4, "B": 4}
STAR_SIZE = 7.0  # 星标大小（pt），沿用各曲线颜色。
AXIS_LINEWIDTH = 0.85
AXIS_FONTWEIGHT = "semibold"
...
def main():
    ...
    for index, (ax, parameter) in enumerate(zip(axes, PARAMETERS)):
        ...
        time_line, = time_ax.plot(x, seconds, color=TIME_COLOR, linestyle="--",
                                 ...
                                 label="RG-LNS time")
        # 从实验记录定位默认点，并核对论文采用的 K/M/B，避免误标最高值。
        default_index = next(i for i, row in enumerate(series)
                             if row["is_default"].lower() == "true")
        if any(int(series[default_index][p]) != value
               for p, value in DEFAULT_PARAMETERS.items()):
            raise ValueError("Recorded defaults differ from paper parameters")
        for star_ax, values, color in (
                (ax, pa, PA_COLOR), (time_ax, seconds, TIME_COLOR)):
            star_ax.plot([x[default_index]], [values[default_index]],
                         linestyle="none", marker="*", markersize=STAR_SIZE,
                         markerfacecolor=color, markeredgecolor="white",
                         markeredgewidth=0.35, zorder=5)
        if index == 0:
            default_handle = Line2D([], [], linestyle="none", marker="*",
                                    markersize=STAR_SIZE, color="#333333",
                                    label="Default")
            legend_handles = [pa_line, time_line, default_handle]
        ...
        ax.tick_params(axis="y", colors=PA_COLOR, length=2.4, pad=2,
                       width=AXIS_LINEWIDTH,
                       left=index == 0, labelleft=index == 0)
        time_ax.tick_params(axis="y", colors=TIME_COLOR, length=2.4, pad=2,
                            width=AXIS_LINEWIDTH,
                            right=index == 2, labelright=index == 2)
        for label in ax.get_yticklabels():
            label.set_fontweight(AXIS_FONTWEIGHT)
        for label in time_ax.get_yticklabels():
            label.set_fontweight(AXIS_FONTWEIGHT)
        ...
        time_ax.spines["right"].set_linewidth(AXIS_LINEWIDTH)
    ...
    fig.legend(handles=legend_handles, loc="upper center",
               bbox_to_anchor=(0.50, 0.94), ncol=3, frameon=False,
